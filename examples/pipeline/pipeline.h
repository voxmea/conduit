...
struct SimInit
{
    ...
    {
        std::string name;
        mix::Function<void(conduit::Registrar &)> init;
    };
...
{
    std::string op;
    mix::Function<void(std::stack<int> &)> exec;
};
...
{
    static const std::unordered_map<std::string, mix::Function<Instr(const std::string &arg)>> instructions = {
        ...
        {"print", [] (const std::string &arg) {
            return Instr{"print", [arg] (std::stack<int> &stack) {fmt::print_colored(fmt::RED, "{} {}\n", arg, stack.top());}};
        }},
