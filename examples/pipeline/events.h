...
#include <mix/function.h>
#include <mix/accordion.h>
#include <limits>
#include <tuple>
...
// const int COMMIT = FLUSH + 1;
const int COMMIT = 0;
const int RESULT = 2;
const int PICK = 3;
const int STEP_LIMIT = 4;
const int POST = std::numeric_limits<int>::max();

struct Event
{
    uint64_t cycle;
    mix::Function<void(), 64> ev;
...
namespace detail
{
    using cycle = mix::FixVec<Event, 1000>;
    using Scheduler = mix::Accordion<cycle>;
...
template <typename T, typename... U> void next(T &&t, U &&... u)
{
    sched(1, std::forward<T>(t), std::forward<U>(u)...);
}
